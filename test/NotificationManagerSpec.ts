...
describe("NotificationManager, given an area and a model id", () => {
    ...
    context("when all notifications are not needed anymore", () => {
        it("should dispose the subscription", () => {
            ...
            let anotherSubscription = subject.notificationsFor(new ModelContext("Admin", "FakePage", { id: 60 })).subscribe();
            subscription.dispose();

            client.verify(c => c.emit("unsubscribe", It.isValue(new ModelContext("Admin", "FakePage", { id: 60 }))), Times.never());
